...
# Версия, зашитая прямо в этот файл. Меняйте её ЗАЕДНО с version.txt
# в репозитории при каждом релизе — по ней main.py узнаёт себя даже
# после автоотката (когда boot.py подменяет main.py файлом main.old,
# а внешний version.txt при этом остаётся "неправильным").
OWN_VERSION = "1"
...
def run_ota_check():
    ...
    # Не повторяем скачивание версии, которая уже откатывалась автоматически -
    # иначе после отката (который поправит local_ver обратно на рабочую версию
    # в check_ota_state) мы бы тут же полезли качать ту же самую битую версию
    # заново, и устройство ушло бы в бесконечный цикл "качаем -> падаем ->
    # откатываемся -> качаем снова" вместо того, чтобы спокойно работать.
    try:
        with open('ota_blacklist.txt', 'r') as f:
            blacklisted_ver = f.read().strip()
    except Exception:
        blacklisted_ver = None

    if blacklisted_ver is not None and blacklisted_ver == remote_ver:
        print(f"Версия {remote_ver} уже откатывалась ранее - повторная попытка пропущена. Опубликуйте новую версию, чтобы повторить.")
        return
    ...
    if download_file_streamed(MAIN_URL, 'main.new'):
        ...
        # Помечаем обновление как "не подтвержденное" - существующий
...
def check_ota_state():
    ...
    if pending:
        ...
        try:
            ...
            print("Не удалось поправить version.txt после отката:", e)
        try:
            with open('ota_blacklist.txt', 'w') as f:
                f.write(recorded_ver)
        except Exception as e:
